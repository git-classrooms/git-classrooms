...
func init() {
	...
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&database.User{}) fail: %s", err)
	}
}
...
func Test_userQuery(t *testing.T) {
	...
	err = _do.Create(&database.User{})
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}

	err = _do.Save(&database.User{})
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}

	err = _do.CreateInBatches([]*database.User{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*database.User{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
	...
	_, err = _do.ScanByPage(&database.User{}, 0, 1)
	if err != nil {
