...
func TestPatchMemberRole(t *testing.T) {
	...
	run := func(r RunTest) {
		...
		route := fmt.Sprintf("/api/v2/classrooms/%s/members/%d/role", classroom.ID.String(), r.user.ID)
		...
		gitlabRepo.
			EXPECT().
			ChangeUserAccessLevelInGroup(classroom.GroupID, r.user.ID, r.accessLevel).
			Return(nil)

		req := newJsonRequest(route, requestBody, "PA")
		...
		updatedUserClassroom, err :=
			query.
				...
				Where(query.UserClassrooms.ClassroomID.Eq(classroom.ID)).
				First()

		assert.Equal(t, r.changeTo, updatedUserClassroom.Role)
	}

	t.Run("The creator of the classroom can not change the role", func(t *testing.T) {
	})
